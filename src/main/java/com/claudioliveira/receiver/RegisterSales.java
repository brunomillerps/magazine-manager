    ...
    public void start() throws Exception {
        ...
        eb.consumer(DomainEvent.NEW_SALE.event(), message ->
                        mongoClient.insert(DomainCollection.SALES.collection(), new JsonObject(message
                            ...
                            if (result.succeeded()) {
                                eb.publish(DomainEvent.UPDATE_DELIVERY_STOCK.event(), new JsonObject(result.result()));
                                eb.publish(DomainEvent.UPDATE_MAGAZINE_STOCK.event(), new JsonObject(result.result()));
                            } else {
